...
/* Import Class */
class Engine_Cloudsponge_CSImport implements iCSConstants {
  /* Constants */
  ...
  static function begin_import($source_name, $username = NULL, $password = NULL, $tracking_string = '', $redirect_url = NULL, $options = array()) {
    ...
    if (!empty($username)) {
      $resp = Engine_Cloudsponge_CSImport::begin_import_username_password($source_name, $username, $password, $tracking_string, $options);
    } else {
      if (strcasecmp($source_name, "OUTLOOK") == 0 || strcasecmp($source_name, "ADDRESSBOOK") == 0) {
        $resp = Engine_Cloudsponge_CSImport::begin_import_applet($source_name, $tracking_string, $options);
        $applet_tag = Engine_Cloudsponge_CSImport::create_applet_tag($resp['id'], $resp['url']);
      } else {
        $resp = Engine_Cloudsponge_CSImport::begin_import_consent($source_name, $tracking_string, $redirect_url, $options);
  ...
  static function begin_import_consent($source_name, $tracking_string = NULL, $redirect_url = NULL, $options = array()) {
    ...
    $resp = Engine_Cloudsponge_CSImport::post_and_decode_response(Engine_Cloudsponge_CSImport::full_url(Engine_Cloudsponge_CSImport::CONSENT_PATH), $params);
  ...
  static function begin_import_applet($source_name, $tracking_string = NULL, $options = array()) {
    ...
    // get and decode the response into an associated array
    // Throws an exception if there was a problem at the server
    $resp = Engine_Cloudsponge_CSImport::post_and_decode_response(Engine_Cloudsponge_CSImport::full_url(Engine_Cloudsponge_CSImport::APPLET_PATH), $params);
  ...
  static function begin_import_username_password($source_name, $username, $password, $tracking_string = NULL, $options = array()) {
    ...
    // get and decode the response into an associated array
    // Throws an exception if there was a problem at the server
    $resp = Engine_Cloudsponge_CSImport::post_and_decode_response(Engine_Cloudsponge_CSImport::full_url(Engine_Cloudsponge_CSImport::IMPORT_PATH), $params);
  ...
  static function get_events($import_id) {
    ...
    $full_url = Engine_Cloudsponge_CSImport::generate_poll_url(Engine_Cloudsponge_CSImport::EVENTS_PATH, $import_id);
    
    // get the response from the server and decode it
    $resp = Engine_Cloudsponge_CSImport::get_and_decode_response($full_url);
  ...
  static function get_contacts($import_id) {
    ...
    // create the appropriate url to fetch the contacts
    $full_url = Engine_Cloudsponge_CSImport::generate_poll_url(Engine_Cloudsponge_CSImport::CONTACTS_PATH, $import_id);
  ...
  static function forward_auth($get_params, $post_params = null) {
    $url = Engine_Cloudsponge_CSImport::URL_BASE . 'auth?' . http_build_query($get_params);
    if (isset($post_params)) {
      if(in_array('appctx', $post_params)) {
        $post_params['appctx'] = stripslashes($post_params['appctx']);
      }
      $response = Engine_Cloudsponge_CSImport::post_url($url, $post_params);
    } else {
      $response = Engine_Cloudsponge_CSImport::get_url($url);
    }
  ...
  static function full_url($path) {
    return Engine_Cloudsponge_CSImport::URL_BASE . Engine_Cloudsponge_CSImport::BEGIN_PATH . $path;
  }
  static function authenticated_params($params = array()) {
    ...
    $params["domain_key"] = Engine_Cloudsponge_CSImport::DOMAIN_KEY;
    $params["domain_password"] = Engine_Cloudsponge_CSImport::DOMAIN_PASSWORD;
    return $params;
  }
  static function authenticated_query($params = array()) {
    return http_build_query(Engine_Cloudsponge_CSImport::authenticated_params($params));
  }
...
EOS;
  ...
  static function generate_poll_url($path, $import_id) {
    ...
    $query_string = Engine_Cloudsponge_CSImport::authenticated_query();
    // assemble the full url
    $full_url = Engine_Cloudsponge_CSImport::URL_BASE . $path .  $import_id . "?" . $query_string;
    // echo $full_url;
    return $full_url;
  }
  static function post_and_decode_response($url, $params) {
    $params = Engine_Cloudsponge_CSImport::authenticated_params($params);
    
    // post the response
    $response = Engine_Cloudsponge_CSImport::post_url($url, $params);
    // decode the response into an asscoiative array
    $resp = Engine_Cloudsponge_CSImport::decode_response($response, 'json');
  ...
  static function get_and_decode_response($full_url) {
    // get the response
    $response = Engine_Cloudsponge_CSImport::get_url($full_url);
  ...
  static function post_url($url, $encoded_params) {
    return Engine_Cloudsponge_CSImport::url_request($url, 'post', $encoded_params);
  }
  static function get_url($url) {
    return Engine_Cloudsponge_CSImport::url_request($url);
  }
  static function url_request($url, $method = 'get', $params = null) {
    // init the curl agent

    $agent = curl_init();
    curl_setopt($agent,CURLOPT_SSL_VERIFYPEER,true); 
    //curl_setopt($agent,CURLOPT_CAINFO,'D:/xampp/apache/conf/ssl.crt/server.crt');
    curl_setopt($agent, CURLOPT_CAINFO,  getcwd().'/curl-ca-bundle.crt');
   // curl_setopt($agent, CURLOPT_SSLCERT, getcwd().'D:\globussoft\cacert.pem\curl-ca-bundle.crt');
